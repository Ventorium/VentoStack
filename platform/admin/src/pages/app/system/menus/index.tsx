...
import {
  Card, Table, Button, Form, Input, Select, Modal, Space, Tag, message, Popconfirm,
} from 'antd'
import { PlusOutlined } from '@ant-design/icons'
import { menuApi } from '@/api/system'
import type { MenuItem, CreateMenuBody, UpdateMenuBody } from '@/api/types'
...
const MenuPage = () => {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true)
    try {
      const { error, data: tree } = await menuApi.getTree()
  ...
  const openCreate = (parent?: MenuItem) => {
    ...
    form.resetFields()
    if (parent) {
      form.setFieldsValue({ parentId: parent.id, type: 2 })
    } else {
      form.setFieldsValue({ type: 1, sort: 0, visible: true, status: 1 })
    }
    setModalOpen(true)
  }
  ...
  const handleOk = async () => {
    ...
    try {
      if (editingMenu) {
        ...
        const { error } = await menuApi.update(editingMenu.id, body)
        if (!error) {
        ...
        const { error } = await menuApi.create(body)
        if (!error) {
  ...
  return (
    <div>
      ...
      <Card
        title="菜单列表"
        extra={<Button type="primary" icon={<PlusOutlined />} onClick={() => openCreate()}>新增菜单</Button>}
      >
        <Table
          rowKey="id"
          columns={columns}
      ...
      >
        <Form form={form} layout="vertical" preserve={false}>
          <Form.Item name="parentId" label="上级菜单">
            <Select allowClear placeholder="顶级菜单">
              <Select.Option value={''}>顶级菜单</Select.Option>
              {data.map(m => renderMenuSelectOptions(m))}
            </Select>
          </Form.Item>
          ...
          <Form.Item name="visible" label="是否显示" initialValue={true}>
            <Select>
...
function renderMenuSelectOptions(menu: MenuItem): ReactNode {
  return (
    <>
      <Select.Option key={menu.id} value={menu.id}>{menu.name}</Select.Option>
      {menu.children?.map(child => renderMenuSelectOptions(child))}
    </>
  )
}
