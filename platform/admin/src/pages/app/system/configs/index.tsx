...
import {
  Card, Table, Button, Input, Select, Form, Modal, Space, message, Popconfirm,
} from 'antd'
import { PlusOutlined, SearchOutlined, ReloadOutlined } from '@ant-design/icons'
import { configApi } from '@/api/system'
import type { ConfigItem, CreateConfigBody, UpdateConfigBody } from '@/api/types'
...
const ConfigPage = () => {
  ...
  const fetchData = useCallback(async (p = page, ps = pageSize) => {
    ...
    try {
      ...
      const { error, data: result } = await configApi.list({ page: p, pageSize: ps, ...values })
      if (!error && result) {
  ...
  const openEdit = (r: ConfigItem) => {
    setEditingConfig(r)
    form.setFieldsValue({ name: r.name, key: r.key, value: r.value, type: r.type, group: r.group, remark: r.remark })
    setModalOpen(true)
  }
  ...
  const handleOk = async () => {
    ...
    try {
      if (editingConfig) {
        const body: UpdateConfigBody = { name: values.name, value: values.value, type: values.type, group: values.group, remark: values.remark }
        const { error } = await configApi.update(editingConfig.key, body)
        if (!error) { message.success('更新成功'); setModalOpen(false); fetchData(page, pageSize) }
      } else {
        const body: CreateConfigBody = { name: values.name, key: values.key, value: values.value, type: values.type, group: values.group, remark: values.remark }
        const { error } = await configApi.create(body)
  ...
  const handleDelete = async (key: string) => {
    const { error } = await configApi.delete(key)
  ...
  const columns = [
    ...
    {
      title: '参数键值', dataIndex: 'value', key: 'value', ellipsis: true,
    ...
    { title: '创建时间', dataIndex: 'createdAt', key: 'createdAt', width: 180 },
    {
      ...
      render: (_: unknown, r: ConfigItem) => (
        <Space>
          ...
          <Popconfirm title="确定删除该参数？" onConfirm={() => handleDelete(r.key)}>
  ...
  return (
    <div>
      ...
      <Card
        title={`参数列表（${total}）`}
        extra={<Button type="primary" icon={<PlusOutlined />} onClick={openCreate}>新增参数</Button>}
      >
        <Table
      ...
      >
        <Form form={form} layout="vertical" preserve={false}>
          <Form.Item name="name" label="参数名称" rules={[{ required: true, message: '请输入名称' }]}>
            <Input />
          </Form.Item>
          <Form.Item name="key" label="参数键名" rules={[{ required: true, message: '请输入键名' }]}>
            <Input disabled={!!editingConfig} />
          </Form.Item>
          <Form.Item name="value" label="参数键值" rules={[{ required: true, message: '请输入键值' }]}>
            <Input.TextArea rows={3} />
          </Form.Item>
          <Form.Item name="type" label="类型" initialValue={0}>
            <Select>
              <Select.Option value={0}>字符串</Select.Option>
              <Select.Option value={1}>数字</Select.Option>
              <Select.Option value={2}>布尔</Select.Option>
              <Select.Option value={3}>JSON</Select.Option>
            </Select>
          </Form.Item>
