...
import { readZipEntries } from '@ventostack/file2md';
...
export function createSkillService(deps: SkillServiceDeps) {
  ...
  function mapSkill(r: Record<string, unknown>): SkillItem {
    ...
    return {
      ...
      installedAt:
        r.installed_at instanceof Date
          ? r.installed_at.toISOString()
          : ((r.installed_at as string) ?? null),
      lastSyncedAt:
        r.last_synced_at instanceof Date
          ? r.last_synced_at.toISOString()
          : ((r.last_synced_at as string) ?? null),
  ...
  async function list(
    tenantId: string,
    params?: { source?: string; enabled?: boolean; page?: number; pageSize?: number },
    ...
    const countRows = (await db.raw(
      `SELECT COUNT(*) as cnt FROM ai_skill WHERE ${where}`,
      values,
  ...
  async function getById(id: string, tenantId: string): Promise<SkillItem | null> {
    const rows = (await db.raw('SELECT * FROM ai_skill WHERE id = $1 AND tenant_id = $2', [
      id,
      tenantId,
    ])) as Array<Record<string, unknown>>;
  ...
  async function getBySlug(slug: string, tenantId: string): Promise<SkillItem | null> {
    const rows = (await db.raw('SELECT * FROM ai_skill WHERE slug = $1 AND tenant_id = $2', [
      slug,
      tenantId,
    ])) as Array<Record<string, unknown>>;
    return rows.length > 0 ? mapSkill(rows[0]) : null;
  ...
  async function setEnabled(id: string, tenantId: string, enabled: boolean): Promise<void> {
    await db.raw(
      'UPDATE ai_skill SET enabled = $1, updated_at = NOW() WHERE id = $2 AND tenant_id = $3',
      [enabled, id, tenantId],
    );
    // 联动：禁用 skill 时，关闭关联 agent 的该 skill
    if (!enabled) {
      await db.raw(
        'UPDATE ai_agent_skill SET enabled = FALSE WHERE skill_id = $1 AND tenant_id = $2',
        [id, tenantId],
      );
    }
  ...
  async function uninstall(id: string, tenantId: string): Promise<void> {
    ...
    // 删除关联
    await db.raw('DELETE FROM ai_agent_skill WHERE skill_id = $1 AND tenant_id = $2', [
      id,
      tenantId,
    ]);
  ...
  async function syncSkill(
    id: string,
    tenantId: string,
  ): Promise<{ updated: boolean; oldVersion: string | null; newVersion: string | null }> {
    const skill = await getById(id, tenantId);
  ...
  async function upgrade(id: string, tenantId: string): Promise<SkillItem> {
    ...
    let skillMdContent: string | null = null;
    try {
      skillMdContent = await store.getFileContent(skill.slug, 'SKILL.md', newVersion);
    } catch {}

    let readmeContent: string | null = null;
    try {
      readmeContent = await store.getFileContent(skill.slug, 'README.md', newVersion);
    } catch {}
    ...
    await eventBus?.emit('ai.skill.upgraded', {
      id,
      slug: skill.slug,
      oldVersion: skill.installedVersion,
      newVersion,
      tenantId,
    });
    return (await getById(id, tenantId))!;
  ...
  async function checkUpdates(
    tenantId: string,
  ): Promise<Array<{ id: string; slug: string; currentVersion: string; latestVersion: string }>> {
  ...
  async function installFromUpload(params: {
    ...
    const readmeEntry = entries.find(
      (e) => e.name === 'README.md' || e.name.endsWith('/README.md'),
    );
  ...
  async function rescanFileTree(
    ...
    // 更新 DB 中的 file_tree
    await db.raw('UPDATE ai_skill SET file_tree = $1 WHERE id = $2 AND tenant_id = $3', [
      JSON.stringify(files),
      skillId,
      tenantId,
    ]);
  ...
  async function writeFileContent(
    ...
    if (filePath === 'SKILL.md') {
      await db.raw('UPDATE ai_skill SET skill_md_content = $1, updated_at = NOW() WHERE id = $2', [
        content,
        skillId,
      ]);
    } else if (filePath === 'README.md') {
      await db.raw('UPDATE ai_skill SET readme_content = $1, updated_at = NOW() WHERE id = $2', [
        content,
        skillId,
      ]);
    }
