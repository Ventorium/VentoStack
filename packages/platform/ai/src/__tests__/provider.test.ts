...
import { getPresets, getPresetById } from "../services/provider-presets";
...
describe("ProviderService", () => {
  ...
  test("listProviders calls SELECT", async () => {
    ...
    mockRows.push([
      { id: "p1", name: "openai", display_name: "OpenAI", api_format: "openai_chat", base_url: "https://api.openai.com/v1", api_key: "sk-xxx", headers: null, extra: null, preset_id: "openai", status: 1, sort: 0, model_count: 5, created_at: new Date(), updated_at: new Date() },
    ]);
    const service = createProviderService({ db });
  ...
  test("syncModels throws when provider has no preset", async () => {
    ...
    const service = createProviderService({ db });
    await expect(service.syncModels("p1", "default")).rejects.toThrow("no preset");
  });
});
