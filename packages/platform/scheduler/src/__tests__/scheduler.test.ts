...
describe('Scheduler Service', () => {
  describe('create', () => {
    test('列出应用已注册的任务处理器', () => {
      const s = setup({ cleanup: async () => {}, 'sync-users': async () => {} });

      expect(s.schedulerService.listHandlers()).toEqual([
        { id: 'cleanup', label: 'cleanup' },
        { id: 'sync-users', label: 'sync-users' },
      ]);
    });

    test('拒绝创建使用未注册处理器的任务', async () => {
      const s = setup({ cleanup: async () => {} });

      await expect(
        s.schedulerService.create({ name: 'Invalid', handlerId: 'missing' }),
      ).rejects.toThrow('处理器未注册');
      expect(s.calls.some((call) => call.text.includes('INSERT'))).toBe(false);
    });

    test('拒绝当前调度引擎无法准确执行的 Cron', async () => {
      const s = setup({ cleanup: async () => {} });

      await expect(
        s.schedulerService.create({
          name: 'Quartz Job',
          handlerId: 'cleanup',
          cron: '0 0 2 * * ?',
        }),
      ).rejects.toThrow('Cron 表达式不受支持');
      await expect(
        s.schedulerService.create({
          name: 'Fixed Time',
          handlerId: 'cleanup',
    ...
    test('创建任务返回 ID', async () => {
      const s = setup({ 'test-handler': async () => {} });
  ...
  describe('update', () => {
    test('拒绝把任务改成未注册处理器', async () => {
      const s = setup({ cleanup: async () => {} });

      await expect(s.schedulerService.update('job-1', { handlerId: 'missing' })).rejects.toThrow(
        '处理器未注册',
      );
