...
function setup() {
  ...
  const menuTreeBuilder = createMenuTreeBuilder({ db, tenantId: 'default' });
...
describe('MenuTreeBuilder', () => {
  ...
  test('buildRoutesForUser returns routes for user with assigned menus', async () => {
    ...
    s.results.set('FROM sys_role WHERE tenant_id', [{ id: 'r1' }]);
    // 3. role -> menus
    s.results.set('FROM sys_role_menu', [{ menu_id: 'm1' }, { menu_id: 'm2' }]);
    // 4. all menus
    s.results.set('FROM sys_menu WHERE tenant_id', [
      {
        id: 'm1',
        parent_id: null,
        name: '系统管理',
