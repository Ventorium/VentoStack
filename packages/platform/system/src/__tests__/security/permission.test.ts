...
describe('Security: Permission', () => {
  ...
  describe('数据权限范围', () => {
    test('assignDataScope 更新角色数据范围', async () => {
      const s = setupRole();
      await s.roleService.assignDataScope('r1', 2, undefined, {
        all: true,
        departmentIds: [],
      }); // 2 = 本部门数据权限
    ...
    test('assignDataScope 自定义范围时关联部门', async () => {
      const s = setupRole();
      s.results.set('FROM sys_dept', [{ id: 'd1' }, { id: 'd2' }, { id: 'd3' }]);
    ...
    test('assignDataScope 非自定义范围时清理历史关联', async () => {
      const s = setupRole();
      await s.roleService.assignDataScope('r1', 1, undefined, {
        all: true,
        departmentIds: [],
      }); // 1 = 全部数据
