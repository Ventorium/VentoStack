...
import { createOperationLogMiddleware, resolveOperationAction } from '../middlewares/operation-log';
...
describe('OperationLogMiddleware', () => {
  ...
  test('legacy method and URL actions can be localized when read', () => {
    expect(resolveOperationAction('PUT', '/api/system/users/string-id/tags')).toBe('分配用户标签');
  });
  ...
  test('platform write routes have Chinese action descriptions', () => {
    ...
    expect(resolveOperationAction('POST', '/api/workflow/tasks/task-1/approve')).toBe('审批通过');
  ...
  test('skips refresh-token endpoint when configured as authentication logging', async () => {
    const auditLog = createMockAuditStore();
    const middleware = createOperationLogMiddleware(auditLog, {
      excludePathPrefixes: ['/api/auth/refresh'],
    });
    await middleware(mockCtx({ method: 'POST', path: '/api/auth/refresh' }), mockNext());
    expect(auditLog.append).not.toHaveBeenCalled();
  });
  ...
  test('reads the authenticated actor after downstream middleware completes', async () => {
    const auditLog = createMockAuditStore();
    const saveToDb = mock(async () => {});
    const middleware = createOperationLogMiddleware(auditLog, { saveToDb });
    const ctx = mockCtx({ method: 'POST' });
    ...
    await middleware(ctx, async () => {
      ctx.user = { id: 'u2', username: 'operator' };
      return new Response(null, { status: 200 });
    });

    expect((auditLog.append as any).mock.calls[0][0].actor).toBe('operator');
    expect(saveToDb.mock.calls[0]?.[0].user_id).toBe('u2');
    expect(saveToDb.mock.calls[0]?.[0].username).toBe('operator');
  });

  test('records only once when the same middleware wraps a module route', async () => {
    const auditLog = createMockAuditStore();
    const saveToDb = mock(async () => {});
    const middleware = createOperationLogMiddleware(auditLog, { saveToDb });
    const ctx = mockCtx({ method: 'POST' });

    await middleware(ctx, () => middleware(ctx, mockNext()));
