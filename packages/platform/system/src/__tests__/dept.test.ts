...
describe('DeptService', () => {
  ...
  test('create with all optional fields', async () => {
    ...
    s.results.set('SELECT status FROM sys_dept', [{ status: 1 }]);
  ...
  test('create with invalid leader user throws', async () => {
    ...
    await expect(s.deptService.create({ name: '技术部', leaderUserId: 'ghost' })).rejects.toThrow(
      '负责人用户不存在或已停用',
    );
  });
  ...
  test('update executes UPDATE with changed fields', async () => {
    const s = setup();
    s.results.set('SELECT id FROM sys_dept', [{ id: 'd1' }]);
  ...
  test('update rejects moving a department under its descendant', async () => {
    const s = setup();
    s.results.set('SELECT id FROM sys_dept', [{ id: 'd1' }]);
    s.results.set('SELECT id, parent_id, status FROM sys_dept', [
      { id: 'd1', parent_id: null, status: 1 },
      { id: 'd2', parent_id: 'd1', status: 1 },
    ]);
    await expect(s.deptService.update('d1', { parentId: 'd2' })).rejects.toThrow(
      '不能将部门移动到自己的子部门下',
    );
  });
  ...
  test('getTree rejects cyclic dirty data', async () => {
    const s = setup();
    s.results.set('sys_dept', [
      { id: 'd1', parent_id: 'd2', name: 'A', sort: 1, status: 1 },
      { id: 'd2', parent_id: 'd1', name: 'B', sort: 2, status: 1 },
    ]);
    await expect(s.deptService.getTree()).rejects.toThrow('部门层级存在循环引用');
  });
});
