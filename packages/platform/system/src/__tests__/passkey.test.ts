...
describe('PasskeyService', () => {
  ...
  describe('beginRegistration', () => {
    ...
    test('stores challenge in cache', async () => {
      ...
      const result = await s.passkeyService.beginRegistration('user-1');
      const cached = await s.cache.get<string>(`tenant:default:passkey_reg:${result.challengeId}`);
  ...
  describe('finishRegistration', () => {
    ...
    test('throws when registration verification fails', async () => {
      ...
      await s.cache.set('tenant:default:passkey_reg:challenge-1', 'reg-challenge-base64', {
        ttl: 120,
      });
  ...
  describe('beginAuthentication', () => {
    ...
    test('throws when user has no passkeys', async () => {
      ...
      s.results.set('sys_passkey WHERE tenant_id', []);
    ...
    test('returns options and challengeId for valid user with passkeys', async () => {
      ...
      const cached = await s.cache.get<string>(`tenant:default:passkey_auth:${result.challengeId}`);
  ...
  describe('finishAuthentication', () => {
    ...
    test('throws and logs audit when verification fails', async () => {
      ...
      await s.cache.set(
        'tenant:default:passkey_auth:challenge-1',
        JSON.stringify({
          challenge: 'auth-challenge-base64',
          userId: 'user-1',
          username: 'admin',
  ...
  describe('removePasskey', () => {
    ...
    test('deletes passkey and logs audit', async () => {
      const s = setup();
      s.results.set('sys_passkey WHERE tenant_id', [{ id: 'pk-1' }]);
      ...
      expect(deleteCall!.params).toEqual(['default', 'pk-1']);
