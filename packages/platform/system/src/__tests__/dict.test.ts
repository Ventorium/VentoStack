...
function setup() {
  ...
  const dictService = createDictService({ db, cache, tenantId: 'default' });
...
describe('DictService', () => {
  ...
  test('deleteType rejects system dict type', async () => {
    ...
    // Mock the SELECT to return system type
    s.results.set('SELECT', [{ is_system: true }]);
    expect(s.dictService.deleteType('sys_status')).rejects.toThrow('系统内置字典类型不可删除');
  });
  ...
  test('updateData rejects system dict data', async () => {
    ...
    expect(s.dictService.updateData('dd1', { label: '新标签' })).rejects.toThrow(
      '系统内置字典数据不可修改',
    );
  });
  ...
  test('deleteData rejects system dict data', async () => {
    ...
    // Mock the SELECT to return system data
    s.results.set('SELECT', [{ type_code: 'sys_status', is_system: true }]);
    expect(s.dictService.deleteData('dd1')).rejects.toThrow('系统内置字典数据不可删除');
  });

  // ===== 404 回归：不存在的资源统一返回 NotFoundError =====

  test('updateType on missing type throws 404', async () => {
    const s = setup();
    s.results.set('SELECT', []);
    expect(s.dictService.updateType('nonexistent', { name: 'x' })).rejects.toThrow(
      '字典类型不存在',
    );
  });

  test('deleteType on missing type throws 404', async () => {
    const s = setup();
    s.results.set('SELECT', []);
    expect(s.dictService.deleteType('nonexistent')).rejects.toThrow('字典类型不存在');
  });

  test('updateData on missing data throws 404', async () => {
    const s = setup();
    s.results.set('SELECT', []);
    expect(s.dictService.updateData('dd404', { label: 'x' })).rejects.toThrow('字典数据不存在');
  });

  test('deleteData on missing data throws 404', async () => {
    const s = setup();
    s.results.set('SELECT', []);
    expect(s.dictService.deleteData('dd404')).rejects.toThrow('字典数据不存在');
  });
});
