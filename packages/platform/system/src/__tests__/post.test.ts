...
function setup() {
  ...
  const postService = createPostService({ db, tenantId: 'default' });
...
describe('PostService', () => {
  ...
  test('list filters by name and code', async () => {
    const s = setup();
    s.results.set('COUNT', [{ count: 1 }]);
    s.results.set('SELECT', [
      { id: 'p1', name: '总经理', code: 'ceo', sort: 1, status: 1, remark: '' },
    ]);
    const result = await s.postService.list({ name: '总经', code: 'ce' });
    expect(result.total).toBe(1);
    const countCall = s.calls.find((c) => c.text.includes('COUNT'));
    expect(countCall?.params).toContain('%总经%');
    expect(countCall?.params).toContain('%ce%');
  });
});
