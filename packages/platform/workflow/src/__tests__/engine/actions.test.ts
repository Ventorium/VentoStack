...
const condNodes: GraphNodeData[] = [
  { id: 'n-start', name: '开始', type: 'start', config: null },
  {
    ...
    type: 'condition',
    config: {
      conditions: [{ field: 'formData.days', operator: '>', value: 3, targetNodeId: 'n-long' }],
...
describe('actions.ts', () => {
  ...
  describe('createTasksForNode', () => {
    ...
    it('should throw when no assignees and not skip', async () => {
      ...
      await expect(createTasksForNode(deps, db, 'inst-1', node, makeCtx())).rejects.toThrow(
        '无可用审批人',
      );
    });
    ...
    it('sequential — should create task for first assignee only', async () => {
      const node = {
        ...
        config: {
          strategy: 'sequential',
          assignee: { mode: 'fixed', userIds: ['u1', 'u2', 'u3'] },
        },
        outgoingEdges: [],
        incomingEdges: [],
      };
  ...
  describe('processNodeCompletion — sequential 依次审批多审批人', () => {
    ...
    it('第一位审批人通过后创建下一位任务（不跳过 u2/u3）', async () => {
      ...
      results.set('sys_workflow_task', [{ id: 't1', assignee_id: 'u1', status: 1 }]);
      const callsBefore = calls.length;
      await processNodeCompletion(deps, db, 'inst-1', graph, 'n-approve', makeCtx());
      // 应创建 u2 的新任务，且不推进节点（无 node_completed 历史）
      const taskInserts = calls
        .slice(callsBefore)
        .filter((c) => c.text.includes('INSERT INTO sys_workflow_task'));
      expect(taskInserts.length).toBe(1);
      expect(taskInserts[0]!.params).toContain('u2');
      const hist = calls
        .slice(callsBefore)
        .filter((c) => c.text.includes('INSERT INTO sys_workflow_history'));
    ...
    it('有人驳回时不创建下一位任务（直接驳回处理）', async () => {
      const graph = seqGraphWithAssignees(['u1', 'u2']);
      // u1 驳回，u2 尚未分配任务
      results.set('sys_workflow_task', [{ id: 't1', assignee_id: 'u1', status: 2 }]);
      await processNodeCompletion(deps, db, 'inst-1', graph, 'n-approve', makeCtx());
      // 不应创建 u2 任务
      const taskInserts = calls.filter((c) => c.text.includes('INSERT INTO sys_workflow_task'));
      expect(taskInserts.length).toBe(0);
  ...
  describe('advanceFromNode', () => {
    ...
    it('should throw when no next node for non-end', async () => {
      ...
      await expect(
        advanceFromNode(deps, db, 'inst-1', graph, 'n-approve', makeCtx()),
  ...
  describe('handleNodeReject', () => {
    ...
    it('return_to_start — should void tasks and re-enter start', async () => {
      ...
      expect(calls.some((c) => c.text.includes('UPDATE sys_workflow_task SET status = 5'))).toBe(
        true,
      );
    });
      ...
      const nodes: GraphNodeData[] = [
        ...
        },
        {
          ...
          config: {
            ...
            assignee: { mode: 'fixed', userIds: ['u2'] },
          },
        },
        { id: 'n-end', name: '结束', type: 'end', config: null },
      ];
