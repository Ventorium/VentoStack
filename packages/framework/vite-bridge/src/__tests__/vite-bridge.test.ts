...
describe('fetchFallback', () => {
  ...
  test('returns null for /api/ prefix (default skip)', async () => {
    ...
    const res = await bridge.fetchFallback(new Request('http://localhost/api/users'), {} as never);
    expect(res).toBeNull();
  });
  ...
  test('respects custom skipPrefixes', async () => {
    ...
    expect(
      await bridge.fetchFallback(new Request('http://localhost/custom-api/data'), {} as never),
    ).toBeNull();

    expect(
      await bridge.fetchFallback(new Request('http://localhost/webhooks/gh'), {} as never),
    ).toBeNull();
...
describe('restart', () => {
  test('restart() keeps bridge functional', async () => {
    ...
    const before = await bridge.fetchFallback(new Request('http://localhost/'), {} as never);
    expect(before).not.toBeNull();

    await bridge.restart();

    const after = await bridge.fetchFallback(new Request('http://localhost/'), {} as never);
