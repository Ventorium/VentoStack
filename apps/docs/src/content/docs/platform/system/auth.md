...
JWT 验签后，system 管理接口还会验证 `sid` 对应的服务端会话、数据库中的用户启用/黑名单状态，并重新加载当前启用角色覆盖 JWT 角色快照。因此撤销 admin、停用、拉黑或强制下线无需等待 access token 自然过期。历史不含 `sid` 的 access token 会返回 401，用户需要重新登录。
