...
export const addWorkflowMenuSeed: Seed = {
  ...
  async run(executor) {
    const existing = await executor(`SELECT id FROM sys_menu WHERE path = '/workflow' AND type = 1`);
    ...
    await executor(
      ...
      [defMenuId, dirId, "流程定义", "/workflow/definitions", "workflow/definitions/index", 2, "workflow:definition:list", "NodeIndexOutlined", 1],
    );
    ...
    await executor(
      `INSERT INTO sys_menu (id, parent_id, name, path, component, redirect, type, permission, icon, sort, visible, status, created_at, updated_at)
       VALUES ($1, $2, $3, $4, $5, NULL, $6, $7, $8, $9, TRUE, 1, NOW(), NOW())`,
      [instanceMenuId, dirId, "我的申请", "/workflow/instances", "workflow/instances/index", 2, "workflow:instance:list", "FileTextOutlined", 2],
    );
    ...
    await executor(
      `INSERT INTO sys_menu (id, parent_id, name, path, component, redirect, type, permission, icon, sort, visible, status, created_at, updated_at)
       VALUES ($1, $2, $3, $4, $5, NULL, $6, $7, $8, $9, TRUE, 1, NOW(), NOW())`,
      [taskMenuId, dirId, "我的审批", "/workflow/tasks", "workflow/tasks/index", 2, "workflow:task:list", "CheckSquareOutlined", 3],
    );
    ...
    if ((adminRole as unknown[]).length > 0) {
      ...
      for (const menuId of [dirId, defMenuId, instanceMenuId, taskMenuId]) {
        await executor(`INSERT INTO sys_role_menu (role_id, menu_id) VALUES ($1, $2)`, [adminRoleId, menuId]);
      }
    }
  },
};
