...
describe('个人中心页', () => {
  ...
  test('MFA 启用: POST /api/auth/mfa/enable 返回 setupData', () => {
    const client = { post: mock(() => Promise.resolve({ error: null, data: { secret: 'ABCDEF', qrCodeUri: 'otpauth://...', recoveryCodes: ['code1', 'code2'] } })) }
  ...
  test('通行密钥注册: begin → finish 流程', () => {
    const client = { post: mock(() => Promise.resolve({ error: null, data: { options: {}, challengeId: 'ch1' } })) }
    // begin
  ...
  test('通行密钥上限 3 个，达到上限后禁用添加按钮', () => {
    const passkeys = [
      { id: '1', name: 'key1', deviceType: 'platform', backedUp: false, createdAt: '', lastUsedAt: null },
      { id: '2', name: 'key2', deviceType: 'platform', backedUp: false, createdAt: '', lastUsedAt: null },
      { id: '3', name: 'key3', deviceType: 'platform', backedUp: false, createdAt: '', lastUsedAt: null },
    ]
  ...
  test('登录记录使用 useTable + /api/system/login-logs', () => {
    const client = { get: mock(() => Promise.resolve({ error: null, data: { items: [], total: 0 } })) }
