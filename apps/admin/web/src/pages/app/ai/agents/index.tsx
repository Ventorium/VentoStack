...
const AGENT_TYPES = [
  {
    label: '智能问答',
    value: 'chatbot',
    description: '知识库检索 + 记忆 + 网络搜索，适合日常问答',
  },
  {
    label: '深度研究',
    value: 'deep_research',
    description: '多轮规划检索、交叉验证，输出带引用的研究报告',
  },
];
        ...
        <div className="max-h-[360px] overflow-auto">
          {items.map((item) => {
            ...
            return (
              <div
                key={item.id}
                className="flex items-start gap-2 rounded-md mb-1"
                style={{
                  padding: '8px 12px',
...
const AgentsPage = () => {
  ...
  const [tools, setTools] = useState<
    Array<{ id: string; name: string; description: string; riskLevel: string }>
  >([]);
  ...
  const [researchMaxSubtaskTurns, setResearchMaxSubtaskTurns] = useState<number | undefined>(
    undefined,
  );
    ...
    client
      ...
      .then(({ data }) => {
        const models = data as
          | Array<{ modelId: string; displayName: string | null; providerName: string }>
          | undefined;
        if (models?.length) {
  ...
  useEffect(() => {
    client
      ...
      .then(({ data }) => {
        const list = data as
          | Array<{ name: string; description: string; riskLevel: string }>
          | undefined;
        if (list?.length) {
  ...
  const handlePublish = async (record: AgentItem) => {
    const { error } = await client.post('/api/ai/agents/:id/publish', {
      params: { id: record.id },
    });
    if (!error) {
  ...
  const columns: ColumnsType<AgentItem> = [
    ...
    },
    {
      ...
      width: 100,
      render: (_, record) =>
        (record.config?.research as { depth?: string } | undefined)?.depth ? (
            ...
            <Form form={form} layout="vertical">
              ...
              {agentType === 'deep_research' && (
                <>
                  ...
                  <Form.Item
                    label="自定义预算"
                    tooltip="覆盖深度预设的计算预算，留空使用预设默认值（子任务上限默认 6、单子任务轮数默认 4）"
                  >
                    <div className="grid grid-cols-2 gap-2">
                      <InputNumber
                        min={1}
                        max={200}
                        placeholder="迭代轮数（预设覆盖）"
              ...
              <Form.Item
                label="欢迎词"
                name="welcomeMessage"
                extra="新会话空态展示；留空使用默认欢迎词"
              >
                <TextArea rows={2} maxLength={500} showCount placeholder="有什么可以帮助你的？" />
              </Form.Item>
              <Form.Item
                label="系统提示词"
                name="systemPrompt"
              ...
              >
                <TextArea
                  rows={6}
                  placeholder="定义 Agent 的行为、角色和能力边界..."
              ...
              >
                ...
                <div className="flex items-center justify-between">
                  ...
                  <InputNumber
                    ...
                    value={memoryMaxHistory}
                    onChange={(v) => setMemoryMaxHistory(v ?? 20)}
                    className="w-[100px]"
                    suffix="条"
                  />
                </div>
                ...
                items={[
                  {
                    ...
                    label: (
                      <span>
                        <ToolOutlined /> 工具 <Tag>{selectedTools.length}</Tag>
                      </span>
                    ),
                    children: (
                      <AbilityPanel
                  ...
                  },
                  {
                    ...
                    label: (
                      <span>
                        <BlockOutlined /> 技能 <Tag>{selectedSkills.length}</Tag>
                      </span>
                    ),
                    children: (
                      <AbilityPanel
                  ...
                  },
                  {
                    ...
                    label: (
                      <span>
                        <ThunderboltOutlined /> MCP <Tag>{selectedMcp.length}</Tag>
                      </span>
                    ),
                    children: (
                      <AbilityPanel
                          ...
                          return (
                            <div>
                              <Space>
                                ...
                                {mcp && (
                                  <Tag
                                    color={mcp.status === 'connected' ? 'green' : 'red'}
                                    className="text-[11px]"
                                  >
                                    {mcp.status === 'connected' ? '已连接' : '未连接'}
                                  </Tag>
                                )}
                                {mcp && mcp.toolCount > 0 && (
                                  <Tag className="text-[11px]">{mcp.toolCount} 工具</Tag>
                                )}
                              </Space>
                              {item.description && (
                                <div>
                  ...
                  },
                  {
                    ...
                    label: (
                      <span>
                        <BookOutlined /> 知识库 <Tag>{selectedKbs.length}</Tag>
                      </span>
                    ),
                    children: (
                      <AbilityPanel
