...
describe('通知公告页', () => {
  ...
  test('草稿和已撤回状态只能提交发布审批', () => {
    const statusMap: Record<number, { label: string; color: string }> = {
      0: { label: '草稿', color: 'default' },
      1: { label: '已发布', color: 'green' },
      2: { label: '已撤回', color: 'orange' },
    };
    const canSubmitApproval = (status: number) => status === 0 || status === 2;
    const canRevoke = (status: number) => status === 1;

    expect(canSubmitApproval(0)).toBe(true);
    expect(canSubmitApproval(2)).toBe(true);
    expect(canSubmitApproval(1)).toBe(false);
  ...
  test('操作列根据状态动态显示按钮', () => {
    const getActions = (status: number) => {
      const actions = ['编辑'];
      if (status === 0) actions.push('上架审批');
      if (status === 1) actions.push('撤回');
      if (status === 2) actions.push('重新提交审批');
      actions.push('删除');
      return actions;
    };
    expect(getActions(0)).toEqual(['编辑', '上架审批', '删除']);
    expect(getActions(1)).toEqual(['编辑', '撤回', '删除']);
    expect(getActions(2)).toEqual(['编辑', '重新提交审批', '删除']);
  });
});
