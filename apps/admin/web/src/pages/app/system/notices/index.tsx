...
const fetcher = (params: Record<string, unknown>) =>
  client.get<PaginatedData<NoticeItem>>('/api/system/notices', { query: params })
...
const NoticePage = () => {
  ...
  const handleOk = async () => {
    ...
    try {
      if (editingNotice) {
        ...
        const { error } = await client.put(`/api/system/notices/${editingNotice.id}`, { body })
  ...
  const handleDelete = async (id: string) => {
    const { error } = await client.delete(`/api/system/notices/${id}`)
    if (!error) { message.success('删除成功'); refresh() }
  }

  const handlePublish = async (id: string) => {
    const { error } = await client.put(`/api/system/notices/${id}/publish`)
    if (!error) { message.success('已发布'); refresh() }
  }

  const handleRevoke = async (id: string) => {
    const { error } = await client.put(`/api/system/notices/${id}/revoke`)
    ...
    { title: '操作', key: 'action', width: 300,
      render: (_: unknown, r: NoticeItem) => (
        <Space>
          ...
          {r.status === 0 && <Button type="link" size="small" onClick={() => handlePublish(r.id)}>发布</Button>}
          {r.status === 1 && <Button type="link" size="small" onClick={() => handleRevoke(r.id)}>撤回</Button>}
  ...
  return (
    <div>
      ...
      <Modal title={editingNotice ? '编辑公告' : '新增公告'} open={modalOpen} onOk={handleOk} onCancel={() => setModalOpen(false)} confirmLoading={modalLoading} destroyOnClose width={700}>
