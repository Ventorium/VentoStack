...
const NoticePage = () => {
  ...
  const handlePublishApproval = async (r: NoticeItem) => {
    const { error: defErr, data: def } = await client.get(
    ...
    },
    {
      ...
      render: (_: unknown, r: NoticeItem) => {
        const items = [
          ...
          ...(r.status === 0
            ? [{ label: '上架审批', onClick: () => handlePublishApproval(r) }]
            : []),
          ...(r.status === 1 ? [{ label: '下架', onClick: () => handleRevoke(r.id) }] : []),
          ...(r.status === 2
            ? [{ label: '重新提交审批', onClick: () => handlePublishApproval(r) }]
            : []),
          ...(r.status !== 1
            ? [
                {
    ...
    <div>
      ...
      <Card className="mb-4">
        <SearchToolbar form={searchForm} onSearch={handleSearch} onReset={handleReset}>
          ...
          <SearchField name="type" width="normal">
            <DictSelect typeCode="sys_notice_type" placeholder="全部类型" allowClear />
          </SearchField>
          <SearchField name="status" width="normal">
            <DictSelect typeCode="sys_notice_status" placeholder="全部状态" allowClear />
          </SearchField>
        </SearchToolbar>
      </Card>
      <Card
