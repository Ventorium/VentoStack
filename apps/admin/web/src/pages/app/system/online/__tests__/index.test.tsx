...
describe('在线用户页', () => {
  ...
  test('API 端点: GET /api/system/monitor/online', () => {
    const client = { get: mock(() => Promise.resolve({ error: null, data: { list: [], total: 0 } })) }
  ...
  test('响应结构: { list: OnlineUser[], total: number }', () => {
    const response = {
      list: [
        { sessionId: 's1', userId: 'u1', username: 'admin', nickname: '管理员', ip: '192.168.1.1', browser: 'Chrome', os: 'macOS', loginAt: '2024-01-01', lastAccessAt: '2024-01-01' },
      ],
      total: 1,
    }
  ...
  test('强制下线成功后刷新数据', () => {
    const client = { delete: mock(() => Promise.resolve({ error: null })) }
    let refreshed = false
    const fetchData = () => { refreshed = true }
