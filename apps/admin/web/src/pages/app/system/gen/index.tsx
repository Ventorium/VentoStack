...
import type { DbTable, GenTable } from "@/api/types";
...
const GenPage = () => {
  ...
  const { loading, data, total, page, pageSize, refresh, onPageChange } =
    useTable<GenTable>(fetcher);
  ...
  const dbTableColumns: ColumnsType<DbTable> = [
    ...
    { title: "引擎", dataIndex: "engine", key: "engine", width: 80 },
    {
      ...
      key: "createTime",
      width: 180,
      render: (_: unknown, r: DbTable) => fmtDate(r.createTime),
    },
    {
      title: "操作",
      key: "action",
      ...
      render: (_: unknown, r: DbTable) => (
        <Button type="link" size="small" onClick={() => handleImport(r.tableName)}>
          导入
        </Button>
      ),
    },
  ...
  const columns: ColumnsType<GenTable> = [
    ...
    { title: "包路径", dataIndex: "packagePath", key: "packagePath", ellipsis: true },
    {
      ...
      key: "createdAt",
      width: 180,
      render: (_: unknown, r: GenTable) => fmtDate(r.createdAt),
    },
    {
      title: "操作",
      key: "action",
        ...
        items={[
          {
            ...
            children: (
              <Card
                title={`可用表（${dbTables.length}）`}
                extra={
                  <Button icon={<ReloadOutlined />} onClick={fetchDbTables}>
                    刷新
                  </Button>
                }
              >
