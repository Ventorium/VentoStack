...
describe('系统参数页', () => {
  ...
  test('系统预设参数不显示删除操作', () => {
    const actions = (isSystem: boolean) => ['编辑', ...(isSystem ? [] : ['删除'])];
    expect(actions(true)).toEqual(['编辑']);
    expect(actions(false)).toEqual(['编辑', '删除']);
  });

  test('AI 链路追踪和用户自注册属于系统预设参数', () => {
    const rows = [
      { key: 'ai_trace_enabled', isSystem: true },
      { key: 'sys_register_enabled', isSystem: true },
    ];
