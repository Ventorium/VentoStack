...
const fetcher = (params: Record<string, unknown>) =>
  client.get("/api/system/posts", { query: cleanParams(params) }) as Promise<{
    error?: unknown;
    data?: PaginatedData<PostItem>;
  }>;
...
const PostPage = () => {
  ...
  const handleOk = async () => {
    ...
    try {
      if (editingPost) {
        const { error } = await client.put("/api/system/posts/:id", {
          params: { id: editingPost.id },
          body: values,
        });
        if (!error) {
          msg.success("更新成功");
  ...
  const handleBatchDelete = () => {
    ...
    Modal.confirm({
      title: "批量删除",
      content: `确定要删除以下 ${selectedRowKeys.length} 个岗位吗？此操作不可恢复。\n${names}`,
      okType: "danger",
      okText: "确定删除",
      onOk: async () => {
        const { error, data } = await client.post("/api/system/posts/batch-delete", {
  ...
  const columns: ColumnsType<PostItem> = [
    ...
    { title: "排序", dataIndex: "sort", key: "sort", width: 60 },
    {
      ...
      key: "createdAt",
      width: 180,
      render: (_: unknown, r: PostItem) => fmtDate(r.createdAt),
    },
    {
      title: "操作",
      key: "action",
      ...
      >
        <Form form={form} layout="vertical" preserve={false}>
          <Row gutter={16}>
            ...
            <Col span={12}>
              <Form.Item name="code" label="岗位标识" rules={[{ required: true }]}>
