...
describe("字典管理页", () => {
  ...
  test("编辑字典数据后应刷新当前字典数据列表", () => {
    ...
    const shouldRefresh = (typeCode: string, typeName: string) =>
      typeCode === currentTypeCode && typeName === currentTypeName;
  ...
  test("新增字典数据时携带 typeCode", () => {
    const currentTypeCode = "sys_status";
    const buildBody = (values: Record<string, unknown>, typeCode: string) => ({
      ...values,
      typeCode,
    });
  ...
  test("字典类型 CRUD: 更新 API PUT /api/system/dict/types/:id", () => {
    ...
    client.put("/api/system/dict/types/:id", {
      params: { id: "gender" },
      body: { name: "性别(新)", status: 1 },
    });
