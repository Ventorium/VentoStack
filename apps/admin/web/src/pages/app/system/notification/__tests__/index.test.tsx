...
describe("消息中心页", () => {
  ...
  test("NotifyTemplate 类型应包含必要字段", () => {
    const tpl = {
      ...
      type: "system",
      channel: "smtp",
      titleTemplate: "通知: {{title}}",
      contentTemplate: "内容: {{content}}",
  ...
  test("NOTIFICATION_API 常量路径正确", () => {
    ...
    expect(NOTIFICATION_API.MESSAGE_READ_BATCH).toBe(
  ...
  test("批量标记已读 API: POST read-batch body 含 messageIds", () => {
    ...
    client.post("/api/system/notification/messages/read-batch", {
      body: { messageIds: selectedRowKeys },
    });
  ...
  test("未读状态显示 Badge processing，已读显示 Badge default", () => {
    const getStatusBadge = (status: string) =>
      status === "UNREAD"
        ? { status: "processing", text: "未读" }
        : { status: "default", text: "已读" };
  ...
  test("搜索参数经 cleanParams 后应过滤空值", () => {
    ...
    expect(cleanParams({ title: "系统", channel: "", status: undefined })).toEqual({
      title: "系统",
    });
    expect(cleanParams({ title: "", channel: "email", status: "READ" })).toEqual({
      channel: "email",
      status: "READ",
    });
  });
