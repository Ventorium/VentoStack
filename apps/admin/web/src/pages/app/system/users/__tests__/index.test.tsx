...
describe("用户管理页", () => {
  ...
  test("搜索参数组合: username, status, deptId", () => {
    ...
    // 仅 username
    expect(cleanParams({ username: "admin", status: undefined, deptId: undefined })).toEqual({
      username: "admin",
    });

    // username + status
    expect(cleanParams({ username: "admin", status: 1, deptId: undefined })).toEqual({
      username: "admin",
      status: 1,
    });

    // 全部参数
    expect(cleanParams({ username: "admin", status: 1, deptId: "d1" })).toEqual({
      username: "admin",
      status: 1,
      deptId: "d1",
    });
    ...
    // 仅 deptId
    expect(cleanParams({ username: "", status: undefined, deptId: "d1" })).toEqual({
      deptId: "d1",
    });
  ...
  test("选择部门树节点后注入 deptId 到搜索参数", () => {
    ...
    // 选中部门节点
    expect(buildSearchWithDept({ username: "admin" }, "d1")).toEqual({
      username: "admin",
      deptId: "d1",
    });
  ...
  test("编辑用户时 form.setFieldsValue 应映射 roles 为 roleIds 数组", () => {
    ...
    expect(mapRolesToForm({ roles: [{ id: "r1" }, { id: "r2" }] })).toEqual({
      roleIds: ["r1", "r2"],
    });
