              ...
              {serverStatus ? (
                <Space orientation="vertical" style={{ width: '100%' }}>
                  <div>
                    ...
                    <div className="text-xs text-gray-400 mt-1">{serverStatus.cpu.model} ({serverStatus.cpu.cores} cores)</div>
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <div className="text-xs text-gray-400 mt-1">
                      {Math.round(serverStatus.disk.used / 1024 / 1024 / 1024)} GB / {Math.round(serverStatus.disk.total / 1024 / 1024 / 1024)} GB
                    </div>
                  </div>
                  <div>
                    <div className="text-sm text-gray-500 mb-1">运行时间</div>
                    <div className="text-base font-medium">{formatUptime(serverStatus.process.uptime)}</div>
                  </div>
                  <div>
                    <div className="text-sm text-gray-500 mb-1">运行时版本</div>
                    <div className="text-xs text-gray-600">Bun: {serverStatus.process.bunVersion}</div>
                    <div className="text-xs text-gray-600">Node: {serverStatus.process.nodeVersion}</div>
                  </div>
                  <div>
                    <div className="text-sm text-gray-500 mb-1">系统信息</div>
                    <div className="text-xs text-gray-600">{serverStatus.os.platform} / {serverStatus.os.arch}</div>
            ...
            <Card title="缓存状态" size="small">
              {cacheStatus ? (
                <Space orientation="vertical" style={{ width: '100%' }}>
                  ...
                  {cacheStatus.version && <Statistic title="Redis 版本" value={cacheStatus.version} />}
                </Space>
              ) : (
                <div className="text-center text-gray-400 py-4">暂无数据</div>
              )}
              ...
              {dataSourceStatus ? (
                <Space orientation="vertical" style={{ width: '100%' }}>
                  <div>
                    ...
                    <Tag color={dataSourceStatus.connected ? 'green' : 'red'}>{dataSourceStatus.connected ? '已连接' : '未连接'}</Tag>
                  </div>
