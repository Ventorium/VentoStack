...
describe('角色管理页', () => {
  test('toTreeData 将菜单列表转为 Tree 数据', () => {
    const toTreeData = (items: Array<{ id: string; name: string; children?: Array<{ id: string; name: string; children?: unknown[] }> }>) =>
      items.map(item => ({
        key: item.id,
        title: item.name,
  ...
  test('collectAllKeys 收集所有节点 key', () => {
    const collectAllKeys = (items: Array<{ id: string; children?: Array<{ id: string; children?: unknown[] }> }>) => {
      const keys: string[] = []
  ...
  test('getDescendantKeys 获取某节点的所有子孙 key', () => {
    ...
    const getDescendantKeys = (items: Array<{ id: string; children?: Array<{ id: string; children?: unknown[] }> }>, targetKey: string): string[] => {
      for (const item of items) {
