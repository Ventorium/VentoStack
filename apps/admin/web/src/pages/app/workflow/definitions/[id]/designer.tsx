...
export default function WorkflowDesignerPage() {
      ...
      .then(({ error, data }) => {
        if (!error && data) {
          ...
          if (graph.nodes.length > 0) {
            const { nodes: n, edges: e } = toFlowElements(
              graph as Parameters<typeof toFlowElements>[0],
            );
            setNodes(n);
            setEdges(e);
    ...
    setNodes([
      {
        ...
        type: 'start',
        position: { x: 400, y: 80 },
        data: { label: '开始', nodeType: 'start', config: null } as FlowNodeData,
      },
      {
        id: endId,
        type: 'end',
        position: { x: 400, y: 500 },
        data: { label: '结束', nodeType: 'end', config: null } as FlowNodeData,
      },
    ]);
    setEdges([]);
    setTimeout(() => rfInstance.current?.fitView({ maxZoom: 0.7, padding: 0.2 }), 50);
  ...
  const onConnect = useCallback(
    (params: Connection) => {
      setEdges((eds) =>
        addEdge({ ...params, id: genId(), type: 'smoothstep', interactionWidth: 20 }, eds),
      );
    },
    [setEdges],
  );
  ...
  return (
    <div className="flex flex-col bg-[#f5f5f5]" style={{ height: 'calc(100vh - 56px)' }}>
      <div
        ...
        style={{ padding: '8px 16px', borderBottom: '1px solid #e8e8e8' }}
      >
        <Space>
          <Button
        ...
        <Space>
          <Button onClick={handleValidate} icon={<CheckCircleOutlined />}>
            校验
          </Button>
      ...
      <div className="flex-1 flex overflow-hidden">
        <div
          className="p-3 overflow-y-auto bg-[#fafafa]"
