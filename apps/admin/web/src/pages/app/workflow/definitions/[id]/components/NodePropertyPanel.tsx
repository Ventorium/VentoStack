...
import { type ConditionItem, type FlowNodeData, NODE_TYPE_META, type NodeConfig } from './types';
...
function useApiOptions() {
  ...
  useEffect(() => {
    client
      .get('/api/system/posts', { query: { pageSize: 200 } })
      .then(({ data }) => {
      ...
      .catch(() => {});
    client
      .get('/api/system/users', { query: { pageSize: 200 } })
      .then(({ data }) => {
        const list =
          (data as { list?: Array<{ id: string; nickname: string; username: string }> })?.list ??
          [];
        setUsers(list.map((u) => ({ value: u.id, label: `${u.nickname || u.username}` })));
      })
      .catch(() => {});
    client
      .get('/api/system/roles', { query: { pageSize: 200 } })
      .then(({ data }) => {
        const list = (data as { list?: Array<{ id: string; name: string }> })?.list ?? [];
    ...
    client
      ...
      .then(({ data }) => {
        const flatten = (
          items: Array<{ id: string; name: string; children?: typeof items }>,
        ...
        setDepts(
          flatten((data as Array<{ id: string; name: string; children?: typeof data }>) ?? []),
        );
      })
      .catch(() => {});
    client
      .get('/api/system/tags/all')
      .then(({ data }) => {
...
export default function NodePropertyPanel({ node, allNodes, onUpdate, onDelete, onClose }: Props) {
  ...
  const handleSave = () => {
    ...
    if (isCondition) {
      config.conditions = (v.conditions ?? []).filter(
        (c: ConditionItem) => c.field && c.targetNodeId,
      );
            ...
            <Form.Item noStyle shouldUpdate={(prev, cur) => prev.assigneeMode !== cur.assigneeMode}>
              {({ getFieldValue }) => {
                ...
                if (mode === 'form_field')
                  return (
                    <Form.Item
                      name="assigneeFormField"
                      label="表单字段名"
                      extra="流程启动时，该字段值作为审批人用户 ID"
                    >
                    ...
                    <>
                      ...
                      <Form.Item
                        ...
                        shouldUpdate={(prev, cur) =>
                          prev.assigneeDeptTraversal !== cur.assigneeDeptTraversal
                        }
                      >
                        {({ getFieldValue }) =>
                          getFieldValue('assigneeDeptTraversal') && (
                            <Form.Item
                              name="assigneeTraversalLevels"
                              label="最大遍历层级"
                              extra="0 表示不限层级"
                            >
                              <InputNumber
              ...
              {(fields, { add, remove }) => (
                <>
                  ...
                  <Button
                    ...
                    block
                    icon={<PlusOutlined />}
                    onClick={() => add({ field: '', operator: '==', value: '', targetNodeId: '' })}
                  >
                    添加条件
                  </Button>
                </>
