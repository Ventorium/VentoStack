...
> 融合 Andrej Karpathy 的系统级编程思维与 Bun 运行时实践。

### 46.1 系统级理解 Bun

- **理解 Bun 的设计哲学**：Bun 不是 Node 的替代品，而是重新思考 JavaScript 运行时的产物
- **追踪数据流**：从 HTTP 请求进入 `Bun.serve()` 到数据库查询 `Bun.sql` 再到响应返回，理解完整路径
- **关注性能特征**：Bun 的哪些 API 是零拷贝？哪些是异步的？哪些有内部缓冲？
- **理解错误模型**：Bun 的错误是抛出异常还是返回错误对象？如何处理连接池耗尽？

### 46.2 第一性原理选择 API

- **质疑每个 npm 包**：Bun 内置能解决的，绝不多装一个包
- **理解底层**：`Bun.sql` 底层是什么协议？`Bun.password` 使用什么算法？
- **简单优先**：能用标签模板 `` sql`...` `` 就不用 ORM 封装
- **显式依赖**：依赖关系必须可读、可跳转、可测试
...
- **像攻击者一样思考**：输入校验是否充分？边界条件是否处理？
- **像维护者一样思考**：6 个月后能理解这段代码吗？
- **像性能工程师一样思考**：是否有不必要的拷贝？是否利用了 Bun 的优化？
- **检查反模式**：
  - ❌ 使用 `fs.readFile` 而不是 `Bun.file()`
  - ❌ 使用 `child_process` 而不是 `Bun.spawn()`
  - ❌ 使用 `crypto.createHash` 而不是 `Bun.CryptoHasher`
  - ❌ 使用 `.query()` 而不是标签模板 `` sql`...` ``

### 46.5 质量准则

1. **正确性 > 性能 > 优雅**：先做对，再做快，最后做漂亮
2. **显式优于隐式**：代码应该清楚地表达意图
3. **简单优于复杂**：能用 Bun 内置就不用第三方包
4. **局部化变更**：修改的影响范围应该可控
5. **可测试性**：难以测试的代码往往设计有问题
6. **可观测性**：关键路径必须有日志、指标或追踪

### 46.6 与 VentoStack 原则的融合

| VentoStack 原则 | Bun 实践 |
|----------------|---------|
| Bun 优先 | 内置 API > Web 标准 > Node 兼容 > 第三方包 |
| 函数式优先 | 纯函数处理数据，副作用显式标记 |
| 显式依赖 | 依赖注入通过参数，不隐式挂载 |
| 编译期安全 | TypeScript 严格模式，禁止 any |
| 默认安全 | 输入校验、错误脱敏、最小权限 |
| 默认可审计 | 关键操作记录日志 |
| 系统级思考 | 理解 Bun 内部实现和性能特征 |
| 第一性原理 | 质疑每个抽象的底层必要性 |
| 正确性优先 | 测试先行，根因修复 |
